...
A bean whose requirement does not match is not enabled and must not participate in CDI typesafe resolution.
...
* `@RequiresCondition` evaluates a custom `Condition`.
* `@RequiresSetting` evaluates a named runtime setting.
* A non-matching bean is not enabled.
* A non-enabled bean does not participate in CDI typesafe resolution.
* Conditional annotations do not make a class discoverable as a CDI bean.
* The setting resolution strategy belongs to the provider.

== Design goal

The main design goal is to improve developer experience without breaking CDI’s programming model.
...
Bean implementations should declare when they are active:
...
The container should handle the activation decision during bootstrap, before typesafe resolution.
